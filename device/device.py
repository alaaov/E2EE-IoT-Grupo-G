...
class Device:
    ...
    def diffie_hellman(self, client, payload):
        ...
        signature = hmac.new(self.key, (f"device id {self.device_id}").encode("UTF-8"),
        ...
        client.publish(f'newDevice/{self.device_id}', bson.dumps(credentials))
        if self.type_sensor == "humidity":
    ...
    def connect(self) -> mqtt.Client:
        ...
        client = mqtt.Client(client_id=f"{self.type_sensor}:{self.device_id}")
    ...
    def encrypt_data(self, cipher, message, additional_data):
        ...
        if encription[1] == "aes":
            ...
            self.client.publish(f"{self.topic}/{self.device_id}", ciphertext + authentication_tag)
        else:
            self.client.publish(f"{self.topic}/{self.device_id}", ciphertext)

        # Publicar los valores en los temas MQTT correspondientes
        if self.type_sensor == "humidity":
            print(f"({self.device_id}) New {self.type_sensor} value: {message}")
    ...
    def run(self, args):
        ...
        # Generamos los datos adicionales que será la DeviceID
        additional_data = hmac.new(self.key, self.device_id.encode("utf-8"), digestmod="sha256").digest()
        # Bucle principal
...
def main():
    ...
    sensor = Device(topic=args.topic, output_function=output_function, args=args)
    sensor.run(args)
