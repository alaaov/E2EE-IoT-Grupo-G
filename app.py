...
@app.route('/unsubscribe/<string:sensor_type>/<string:device_id>', methods=['GET'])
def unsubscribe(sensor_type, device_id):
    print(f"security/{sensor_type}/{device_id}")
    client.unsubscribe(f"security/{sensor_type}/{device_id}")
    if device_id in devices:
        devices.pop(device_id)
    unsub_devices.append(device_id)
    return redirect('/')


def ChooseAlgorithm(algo, key, nonce):
    algo = algo.split("/")
    if algo[0] == 'ae':
        case = algo[1]
        if case == "aes":
...
def register_device(message, client, device_id):
    if device_id not in unsub_devices:
        ...
        shared_key = handle_sensor_public_key(serialized_sensor_public_key, device_id)

        cipher = ChooseAlgorithm(algo, shared_key, nonce)
        algo = algo.split("/")
        devices[device_id] = {
            "Device_id": device_id,
            "sensor_type": decoded_message["sensor_type"].decode("utf-8"),
        ...
        signature = hmac.new(shared_key, (f"device id {device_id}").encode("UTF-8"), digestmod="sha256").digest()

        # check if device is who it claims to be
        try:
            if signature == decoded_message["signature"]:
                print(f"Sensor {device_id} succesfully authenticated")
                print(f"Received new login from device {device_id}")
                devices[device_id]["authenticated"] = True
            else:
                print("Invalid Authentication")
                devices[device_id]["authenticated"] = False
        except:
            print("An exception occurred tryng to authenticate...")


def handle_sensor_public_key(serialized_sensor_public_key, device_id):
...
def decrypt_data(message, device_id):
    key = devices[device_id]["shared_key"]
    encription = devices[device_id]["encryption"]
    cipher = devices[device_id]["cipher"]
    algo = encription[1]
    ...
    if encription == "aead":
        ...
        additional_data = hmac.new(key, device_id.encode("utf-8"), digestmod="sha256").digest()
...
def handle_mqtt_message(client, userdata, message):
    ...
    if topic[0] == "newDevice":
        device_id = topic[1]
        register_device(message.payload, client, device_id)
        return

    # Identificamos el tipo de sensor y donde debe mandar los datos en la interfaz web
    submit = identify_sensor(topic[1])
    device_id = topic[2]
    if device_id in devices and devices[device_id]["authenticated"] == True:
        plaintext = decrypt_data(message, device_id)
        devices[device_id]["last_value"] = plaintext
        if plaintext != "NO":
            print(f'({device_id}) Received new {submit[3:len(submit) - 4].lower()} value: {plaintext}')
            data = dict(
                topic=message.topic,
                payload=plaintext
            )
        ...
        print(f"Sensor {device_id} not authenticated...")
...
if __name__ == '__main__':
    ...
    thread = threading.Thread(target=RetationKey)
    thread.daemon = True
    thread.start()
    client.loop_start()
